...
_FO4_SYSTEM_CONTEXT = (
    ...
    "- NIF export crashes with 'pack_compressed_mesh: max 255 verts/quads per section': fixed --\n"
    "  Custom Collision (Exact Mesh), Generate Collision Mesh, and LOD-based collision all now\n"
    "  automatically decimate down to fit FO4's real Havok collision limit -- 255 vertices AND\n"
    "  255 triangles, two separate constraints (a first fix only handling vertex count still\n"
    "  crashed on the triangle one) -- when the source mesh is detailed enough to exceed it\n"
    "- A button does nothing and the console shows \"UILayout.operator(): unknown\n"
    "  operator '...'\" even though the feature is documented: this was a real class of bug\n"
    "  where one failing class silently took an entire module's other operators down with\n"
    "  it -- fixed across every affected file; if it still happens, check the System Console\n"
    "  right after Blender starts for a line starting with '⚠ Failed to register' naming the\n"
    "  real cause\n"
